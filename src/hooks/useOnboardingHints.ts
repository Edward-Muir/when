 ...
 *
 * Order: the idle drag nudge (no drag within `DRAG_NUDGE_MS` of play starting) → the
 * first-wrong / first-correct strips, once the placement animation has settled → the
 * swap-button hint, which is the advanced one and waits until the others have had their
 * turn (drag and correct seen, and either wrong seen or four cards placed, so a perfect run
 * still gets it). There is deliberately no rules modal first: it was an essay nobody read,
 * and the strips teach the same loop at the moment each part matters.
 *
...
export function useOnboardingHints(args: UseOnboardingHintsArgs): OnboardingHints {
  ...
  // 1. The idle drag nudge, `DRAG_NUDGE_MS` after play starts with no drag.
  useEffect(() => {
  ...
  // 4. The animation settles: show the outcome's hint, unless the game just ended (then it
  ...
  // 5. The swap hint, after the others, with a quiet gap first.
  const swapEligible =
    playing &&
    !isMultiplayer &&
    !isAnimating &&
  ...
  // 6. Leaving play (game over, or the screen unmounting) clears everything.
  useEffect(() => {
    if (playing) return;
    clearHideTimer();
    setActive(null);
