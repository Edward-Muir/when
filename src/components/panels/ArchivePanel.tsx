 ...
 * there is one, closes the list as a locked card — replayable from tomorrow.
 *
...
const ArchivePanel: React.FC<ArchivePanelProps> = ({
  ...
  const replayable = entries.filter((entry) => !entry.locked).length;

  return (
    <div className="flex flex-1 min-h-0 flex-col">
      {/* Header — styled like the My Timeline heading */}
      <div className="px-3">
        <div className="mx-auto flex w-full max-w-sm flex-wrap items-center justify-between gap-2 py-5">
          <h1 className="font-display text-2xl font-bold text-text">Archive</h1>
          <span className="font-mono text-sm text-text-muted">
            {replayable} {replayable === 1 ? 'deck' : 'decks'}
          </span>
        </div>
      </div>
            ...
            {entries.map((entry) => {
              ...
              return (
                <ArchiveDeckRow
                  ...
                  best={best}
                  playable={!entry.locked && entry.cardCount >= REPLAY_MIN_POOL}
