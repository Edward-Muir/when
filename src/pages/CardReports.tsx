...
const CardReports: React.FC = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('/api/card-reports/list', { cache: 'no-store' }).then(async (res) => {
  ...
  return (
    <div className="min-h-dvh bg-bg text-text">
      ...
      <div className="px-4 py-4 max-w-2xl mx-auto">
        ...
        {!loading && !error && counts && counts.length > 0 && (
          <>
            <p className="text-text-muted font-body text-sm mb-3">
              {counts.length} {counts.length === 1 ? 'card' : 'cards'} reported · most reported
              first
            </p>
              ...
              {counts.map(({ eventName, count }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="font-mono font-bold text-lg text-accent shrink-0">
                      {count}
                    </span>
                  </li>
                );
                ...
                return (
                  ...
                  >
                    <span className="text-text">
                      {event ? event.friendly_name : report.eventName}
                    </span>{' '}
