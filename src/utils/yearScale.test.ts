...
describe('yearScale', () => {
  it('runs cool (1) at the present and warm (0) in deep time', () => {
    expect(yearScale(2025)).toBeCloseTo(1, 5);
    expect(yearScale(-8000)).toBeCloseTo(0, 2);
    expect(yearScale(-4_500_000_000)).toBe(0);
  });
  ...
  it('is monotonic — a later year is never warmer than an earlier one', () => {
    ...
    for (let i = 1; i < years.length; i++) {
      expect(yearScale(years.at(i) ?? 0)).toBeGreaterThanOrEqual(yearScale(years.at(i - 1) ?? 0));
    }
  });
  ...
  it('separates every year inside the ramp, so no two eras share a tone', () => {
    ...
    for (let i = 1; i < years.length; i++) {
      expect(yearScale(years.at(i) ?? 0)).toBeGreaterThan(yearScale(years.at(i - 1) ?? 0));
    }
  });

  it('clamps at both ends rather than running off the ramp', () => {
    expect(yearScale(30000)).toBe(1); // the catalogue has an event past the present
    expect(yearScale(-1e9)).toBe(0);
  });
  ...
  it('gives the crowded post-1500 half most of the ramp', () => {
    ...
    expect(yearScale(1500)).toBeGreaterThan(0.3);
    expect(yearScale(1500)).toBeLessThan(0.5);
  });
});
