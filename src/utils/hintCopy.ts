 ...
 *
 * `reviewEye` is the exception to "one per tab": it names the Daily card's eye, which the
 * tab only grows once today's game is done, so it shares the Daily strip's slot with
 * `dailyTab` rather than having a tab of its own.
 *
...
export const TAB_HINT_TEXT: Record<TabHintKey, string> = {
  ...
  reviewEye: 'Tap the eye to look at your timeline.',
};
