...
An info button beside a card's title in its detail popup swaps the short description for 2-3 paragraphs of long-form prose. Everything between the title and the "Report an issue" row then scrolls as one region, image included, inside the height it already occupied — so the card never changes size. It is on placed cards and everything in My Timeline; the Daily hero carries the same control watermarked into its image (its event is the deck's already-face-up starting card) and opens straight onto the prose. The prose is a lazily-fetched sidecar under `public/events/detail/`, never inlined into the event JSON, and the button is gated so it is unreachable before a card is placed — which is what lets that prose state dates when the short `description` may not. Before touching `src/utils/eventDetail.ts`, `GamePopup`, or `scripts/events/detail-*.js`: see [docs/event-detail/index.md](docs/event-detail/index.md). The corpus itself is unwritten — Phases 2 and 3 of that plan.
