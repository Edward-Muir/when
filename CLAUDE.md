...
A placed card's detail popup shows two paragraphs of long-form prose in place of its short description — no control, no toggle, a ✕ to close. Everything between the title and the "Report an issue" row is one fixed-height scroll region, image included, so every detail card is the same size. This covers placed cards, everything in My Timeline, and the Daily hero, which carries a watermarked (i) on its image (its event is the deck's already-face-up starting card). The description is the fallback wherever the prose is unavailable — not written, or a shard that would not load — so the feature works against a partly written corpus. The prose is a lazily-fetched sidecar under `public/events/detail/`, never inlined into the event JSON, and the button is gated so it is unreachable before a card is placed — which is what lets that prose state dates when the short `description` may not. Before touching `src/utils/eventDetail.ts`, `GamePopup`, or `scripts/events/detail-*.js`: see [docs/event-detail/index.md](docs/event-detail/index.md). **Before writing any of the prose itself**, see [docs/event-detail/writing-spec.md](docs/event-detail/writing-spec.md) and the `write-event-detail` skill — the length band (exactly two paragraphs, 480-830 chars) and a list of banned machine tells (no em dashes, no second person, no puffery) are enforced by `scripts/events/detail-spec.js`, so a batch that ignores them fails the corpus test. The prose is researched rather than recalled: draft, check with a search or two, cut what the results do not support, and never pass `wikipedia_url` to a writer (it is a difficulty-scoring byproduct and it is unreliable). Phase 3 batches run on the `event-detail-writer` sub-agent. 10 of the 5,460 entries are written; the rest is Phase 3.
