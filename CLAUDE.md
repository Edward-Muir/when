...
A placed card's detail popup shows two paragraphs of long-form prose in place of its short description — no control, no toggle, a ✕ to close. Everything between the title and the "Report an issue" row is one fixed-height scroll region, image included, so every detail card is the same size. This covers placed cards, everything in My Timeline, and the Daily hero, which carries a watermarked (i) on its image (its event is the deck's already-face-up starting card). The description is the fallback wherever the prose is unavailable — a shard that would not load, or an event added to the catalogue from here on — which is what let the feature ship against a partly written corpus. The prose is a lazily-fetched sidecar under `public/events/detail/`, never inlined into the event JSON, and the button is gated so it is unreachable before a card is placed — which is what lets that prose state dates when the short `description` may not. Before touching `src/utils/eventDetail.ts`, `GamePopup`, or `scripts/events/detail-*.js`: see [docs/event-detail/index.md](docs/event-detail/index.md). **Before writing any of the prose itself**, see [docs/event-detail/writing-spec.md](docs/event-detail/writing-spec.md) and the `write-event-detail` skill — the length band (exactly two paragraphs, 480-830 chars) and a list of banned machine tells (no em dashes, no second person, no puffery) are enforced by `scripts/events/detail-spec.js`, so a batch that ignores them fails the corpus test. The prose is researched rather than recalled: draft, check with a search or two, cut what the results do not support, and never pass `wikipedia_url` to a writer (it is a difficulty-scoring byproduct and it is unreliable). **All 5,460 entries are written** (2026-09-17) and `node scripts/events/detail-report.js` exits 0; never run `scripts/events/detail-placeholder.js` against a complete corpus. Writing more prose — for a newly added event — runs on the `event-detail-writer` sub-agent. Reading 5,460 records against a source also turned up event records the sources do not support: [docs/events-images/catalogue-error-backlog.md](docs/events-images/catalogue-error-backlog.md) is that backlog, and worth checking before "fixing" an event.
...
- **Onboarding hints**: every one-shot hint (the in-game strips, the home tabs' first-visit strips) is keyed in one object, `when-hints-seen`, in `src/utils/playerStorage.ts`, with a read-only fallback to the key it replaced. The in-game logic is `src/hooks/useOnboardingHints.ts`, not `Game.tsx`, because Game sits on the ESLint `complexity` ceiling; the copy is `src/utils/hintCopy.ts`; the modal is the rules only, and each tab explains itself. See [docs/ui-redesign/index.md](docs/ui-redesign/index.md#onboarding-hints-2026-09) before adding a hint.
