...
Daily themes can be hand-authored: a named list of event slugs pinned to explicit dates. The decks live in `themes/bank.json`, the calendar that pins them to dates lives in Redis, and a GitHub Action schedules the bank onto a cadence (`themes: all`, `start`, `every_days: 7` books a run of Sundays in one dispatch). Before touching `src/utils/dailyTheme.ts`, `dailyPool.ts`, the bank or the daily's deck options: see [docs/curated-themes/index.md](docs/curated-themes/index.md) — the curated lookup must stay ahead of the seeded RNG, both builder call sites must share `getDailyBuildOptions`, and bank order must stay adjacent-disjoint because a weekly cadence lands each deck exactly `RECENCY_DAYS` after the last.
...
- **Routes**: 16 routes in `src/index.tsx`, plus a catch-all redirect to `/`. Player-facing: `/`, `/daily`, `/challenge/:code`, `/stats`, `/achievements`, `/timeline`, `/privacy`, `/terms`, `/support`. Unlinked maintainer tools: `/image-qc`, `/card-reports`, `/cards-preview`, `/unlock-preview`, `/anim-jig`, `/reminder-preview`, `/share-preview`. A new client route also needs a rewrite in `vercel.json` or it 404s on direct load in production — **`/stats`, `/achievements`, `/timeline`, `/cards-preview`, `/unlock-preview`, `/anim-jig`, `/reminder-preview` and `/share-preview` currently have no rewrite**. Of those, `/stats`, `/achievements` and `/timeline` still work via in-app navigation (`TopBar.tsx` routes to them); `/cards-preview`, `/unlock-preview`, `/anim-jig`, `/reminder-preview` and `/share-preview` have **no rewrite and no in-app link**, so they are unreachable in production and only usable in local dev.
