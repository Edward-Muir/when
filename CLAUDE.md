...
- **Routes**: 16 routes in `src/index.tsx`, plus a catch-all redirect to `/`. Player-facing: `/`, `/daily`, `/challenge/:code`, `/stats`, `/achievements`, `/timeline`, `/privacy`, `/terms`, `/support`. Unlinked maintainer tools: `/image-qc`, `/card-reports`, `/cards-preview`, `/unlock-preview`, `/anim-jig`, `/reminder-preview`, `/share-preview`. A new client route also needs a rewrite in `vercel.json` or it 404s on direct load in production — **`/cards-preview`, `/unlock-preview`, `/anim-jig`, `/reminder-preview` and `/share-preview` currently have no rewrite and no in-app link**, so they are unreachable in production and only usable in local dev. `/achievements` and `/timeline` are reached from the burger menu (`Menu.tsx`), `/stats` from the TopBar on those pages; all three have rewrites.
