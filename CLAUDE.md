...
- **Onboarding hints**: every one-shot hint (the How-to-Play modal on the first game, the in-game strips, the home tabs' first-visit strips) is keyed in one object, `when-hints-seen`, in `src/utils/playerStorage.ts`, with read-only fallbacks to the two keys it replaced. The in-game logic is `src/hooks/useOnboardingHints.ts`, not `Game.tsx`, because Game sits on the ESLint `complexity` ceiling; the copy is `src/utils/hintCopy.ts` and is repeated in `HowToPlayModal`. See [docs/ui-redesign/index.md](docs/ui-redesign/index.md#onboarding-hints-2026-09) before adding a hint.
